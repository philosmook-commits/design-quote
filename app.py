...
def connect_google_sheet():
    try:
        # 스트림릿 웹 설정에 저장할 'gcp_service_account' 정보를 가져옵니다.
...
user_name = st.sidebar.text_input("담당자 성함", "홍길동") # 담당자 필드 추가
...
storage_total = storage_fee * 10 # 예시: 10개 PL 고정 보관 시
...
if st.button("🚀 견적 확정 및 구글 시트 저장"):
    ...
    if sheet:
        ...
        st.success(f"✅ '{customer_name}' 견적 데이터가 구글 시트(design견적DB)에 실시간으로 기록되었습니다!")
        st.balloons()
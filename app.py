...
# 7. 그래프 시각화 (가로 막대형으로 변경하여 글자 가로 고정)
st.divider()
st.subheader("📊 항목별 비용 구성 분석")

# 1. 데이터를 표 형태로 정리 (위에서부터 인건비가 나오도록 순서 조정)
chart_data = pd.DataFrame({
    "항목": ['인건비', '보험료', '보관료', '마진'],
    "금액": [total_labor, total_insurance, storage_total, profit]
})

# 2. 가로 막대 그래프 그리기 (st.bar_chart에서 x와 y를 바꾸면 가로형이 됩니다)
# x축에 '금액'을, y축에 '항목'을 넣으면 글자가 가로로 예쁘게 나옵니다.
st.bar_chart(
    data=chart_data, 
    x="금액", 
    y="항목", 
